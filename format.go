...
func decodeKV(data []byte) (uint32, string, string) {
	timestamp, key_len, value_len := decodeHeader(data)

	keyBytes := data[headerSize:headerSize+key_len]
	valueBytes := data[headerSize+key_len:headerSize+key_len+value_len]
